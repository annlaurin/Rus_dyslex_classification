...
def nested_cv_rfe(feature_folds, label_folds, n_folds, c_params, n_features):
    ...
    return final_scores_mean, final_scores_std, best_params, best_n_features, CV_eval, saving_scores
...
sacc_file = pd.read_csv('data/baseline_dataset.csv') 
...
final_scores, final_scores_std, best_params, best_n_features, CV_eval, saving_scores = nested_cv_rfe(feature_folds, 
                                                                    label_folds, n_folds, c_params,
                                                                    n_features)

folder = "baseline_results/nograde/"
dirExist = os.path.exists(folder)
if not dirExist:
    os.makedirs(folder)
    
pred_level = 'readerpred' if BATCH_SUBJECTS else 'sentencepred'
...
with open(f"{folder}aucs_{setting}.pickle" , 'wb') as handle:
    pickle.dump(aucs, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open(f"{folder}CV_eval_{setting}.pickle" , 'wb') as handle:
    pickle.dump(CV_eval, handle, protocol=pickle.HIGHEST_PROTOCOL) 

with open(f"{folder}label_folds_{setting}.pickle" , 'wb') as handle:
    pickle.dump(label_folds, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open(f"{folder}best_n_features_{setting}.pickle" , 'wb') as handle:
    pickle.dump(best_n_features, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open(f"{folder}saving_scores_{setting}.pickle" , 'wb') as handle:
    pickle.dump(saving_scores, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open(f"{folder}best_params_{setting}.pickle" , 'wb') as handle:
